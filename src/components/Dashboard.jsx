...
export default function Dashboard({ sensorData, isCloudOnline, sendCommand }) {
  ...
  // Emergency Override Speed State (ADDED THIS LINE)
    ...
    <div className="container">
      ...
      <div className="card">
        ...
        <div className="sensor-container">
          ...
          <SensorBox
            label="Humidity"
            value={sensorData.humidity ? `${sensorData.humidity}%` : "--%"}
          />
          <SensorBox
            label="Soil Moisture"
          ...
          <SensorBox
            label="Pump Status"
            value={sensorData.pumpStatus || "OFF"}
          />
          <div className="sensor-box salinity-live-box">
            ...
            <div className="sensor-val">
              {currentSalinity !== null
                ? `${currentSalinity.toFixed(1)} mS`
                : "— mS"}
            </div>
            <div className={`sensor-sublabel ${salInfo?.cls}-text`}>
              {salInfo?.label || "Not Read"}
            </div>
          </div>
        </div>
        {isDry && masterMode === "manual" && (
          <p className="live-warning">
            ⚠️ Alert: Soil moisture is critically low! Manual irrigation
            required.
          </p>
        )}
        {salInfo?.level === "high" && (
          <p className="live-warning sal-warning">
            🧪 Alert: High soil salinity! Irrigation quantities are being
            auto-reduced.
          </p>
        )}
      </div>
      ...
      <div className="card salinity-card">
        <div className="card-header">
          ...
          <span className={`badge ${salInfo?.cls || "sal-unread"}`}>
            {salInfo?.label || "— Not Read"}
          </span>
        </div>
        <p className="subtitle">
          One-time soil electrical conductivity (EC) reading. Affects water
          quantity and drip speed.
        </p>
        ...
        <button className="btn-salinity" onClick={handleSetSalinity}>
          Set Salinity Value
        </button>
        ...
        {salInfo && (
          <div className="salinity-info-panel">
            <div className="sal-ec-display">
              ...
              <span className="sal-ec-value">
                {currentSalinity.toFixed(2)} mS/cm
              </span>
            </div>
            <div className="sal-impact-grid">
              <div className="sal-impact-box">
                <div className="sal-impact-label">💧 Surface Irrigation</div>
                <div className="sal-impact-val">
                  {salInfo.factor === 1
                    ? "No change"
                    : `−${Math.round((1 - salInfo.factor) * 100)}% volume`}
                </div>
              </div>
              <div className="sal-impact-box">
      ...
      <div className="card">
        ...
        <div className="master-toggle">
          ...
          <input
            ...
            id="modeAuto"
            name="masterMode"
            checked={masterMode === "auto"}
            onChange={() => setMasterMode("auto")}
          />
      ...
      {masterMode === "auto" && (
        <div className="card auto-card">
          ...
          <p className="subtitle">
            System monitors soil, temperature, humidity, and salinity to
            calculate precise water needs.
          </p>
          <form onSubmit={handleAutoSubmit}>
            <div className="form-group">
              ...
              <select
                value={autoPlantType}
                onChange={(e) => setAutoPlantType(e.target.value)}
              >
              ...
              <div className="radio-group-box">
                <label className="radio-group">
                  <input
                    type="radio"
                    name="autoMethod"
                    checked={autoMethod === "drip"}
                    onChange={() => setAutoMethod("drip")}
                  />
                ...
                <label className="radio-group">
                  <input
                    type="radio"
                    name="autoMethod"
                    checked={autoMethod === "surface"}
                    onChange={() => setAutoMethod("surface")}
                  />
        ...
        <>
          ...
          {manualMode !== "none" && (
            <div className="card smart-guide">
              ...
              <p className="subtitle">
                Select a plant type and we'll auto-fill the optimal settings for
                you.
              </p>
              <select
                value={plantDataset}
                onChange={(e) => setPlantDataset(e.target.value)}
              >
              ...
              <form onSubmit={handleDripSubmit}>
                <div className="input-section">
                  <label className="checkbox-label">
                    <input
                      type="checkbox"
                      checked={dripStartNow}
                      onChange={(e) => setDripStartNow(e.target.checked)}
                    />
                  ...
                  {!dripStartNow && (
                    <div className="form-group" style={{ marginTop: "14px" }}>
                      ...
                      <input
                        type="time"
                        value={dripStartTime}
                        onChange={(e) => setDripStartTime(e.target.value)}
                        required
                      />
                    </div>
                  )}
              ...
              <form onSubmit={handleSurfaceSubmit}>
                <div className="input-section">
                  <label className="radio-group">
                    <input
                      type="radio"
                      checked={surfaceControl === "volume"}
                      onChange={() => setSurfaceControl("volume")}
                    />
                ...
                <div className="input-section">
                  <label className="radio-group">
                    <input
                      type="radio"
                      checked={surfaceControl === "time"}
                      onChange={() => setSurfaceControl("time")}
                    />
                  ...
                  {surfaceControl === "time" && (
                    ...
                    >
                      <label className="checkbox-label">
                        <input
                          type="checkbox"
                          checked={surfaceStartNow}
                          onChange={(e) => setSurfaceStartNow(e.target.checked)}
                        />
                        &nbsp;Start Immediately (Now)
                      </label>
                      {!surfaceStartNow && (
                        <div className="form-group">
                          ...
                          <input
                            type="time"
                            value={surfaceStartTime}
                            onChange={(e) =>
                              setSurfaceStartTime(e.target.value)
                            }
                            required
                          />
                        </div>
      ...
      <div className="card override-card">
        ...
        <div className="btn-group">
          ...
          <button
            ...
            onClick={() => sendCommand("OVERRIDE", { state: "OFF" })}
          >
            ⛔ FORCE PUMP OFF
          </button>
        </div>
